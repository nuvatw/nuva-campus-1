...
import type { SupportStats } from '@/app/types/supporter';
...
function FlipCard({ stat, rank, isTop3 = false }: FlipCardProps) {
  ...
  const getRankStyle = () => {
    if (rank === 1) return 'from-accent to-amber-600';
      ...
      >
        ...
        <div
          className={`absolute inset-0 backface-hidden rounded-xl bg-gradient-to-br ${getRankStyle()}
            flex items-center justify-center gap-2 text-white shadow-elevation-2 px-4`}
        >
        ...
        {/* Back - university */}
        <div
          className="absolute inset-0 backface-hidden rotate-y-180 rounded-xl bg-bg-card border border-border
            flex items-center justify-center gap-2 px-3 shadow-elevation-2"
        >
