...
import { DarkModeToggle } from '@/app/components/operational';
    ...
    <div className="py-8 px-6">
      <div className="max-w-4xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          <div className="card animate-shimmer"><div className="h-32 rounded" /></div>
          <div className="card animate-shimmer"><div className="h-32 rounded" /></div>
        </div>
        <div className="card animate-shimmer"><div className="h-24 rounded" /></div>
      </div>
    </div>
  );
}
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <StaggerChildren className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          <StaggerChildren.Item>
            <Link href={`/guardian/events/${eventId}/checkin`} className="card card-interactive block">
              <div className="text-center">
                <h3 className="text-lg font-medium text-text-primary mb-4">報到</h3>
                <div className="w-16 h-px bg-border mx-auto mb-4" />
                <div className="text-4xl font-semibold text-primary mb-2 tabular-nums">
                  <AnimatedCounter value={stats.checkedIn} />
          ...
          <StaggerChildren.Item>
            <Link href={`/guardian/events/${eventId}/lunch`} className="card card-interactive block">
              <div className="text-center">
                <h3 className="text-lg font-medium text-text-primary mb-4">便當領取</h3>
                <div className="w-16 h-px bg-border mx-auto mb-4" />
                <div className="text-4xl font-semibold text-primary mb-2 tabular-nums">
                  <AnimatedCounter value={stats.lunchCollected} />
        ...
        {/* Notifications */}
        <FadeIn delay={0.15}>
          <Link href={`/guardian/events/${eventId}/notify`} className="card card-interactive block">
            ...
            <div className="flex gap-8">
              <div>
                ...
                <p className="text-2xl font-semibold text-text-primary tabular-nums">
                  <AnimatedCounter value={stats.notificationSent} />
                </p>
              </div>
              <div>
                <p className="text-sm text-text-secondary mb-1">未發送</p>
                <p className="text-2xl font-semibold text-warning tabular-nums">
                  <AnimatedCounter value={stats.notificationPending} />
                </p>
              </div>
            </div>
            <div className="mt-4 flex justify-end">
              <span className="text-primary text-sm font-medium">管理通知 →</span>
            </div>
          </Link>
        </FadeIn>
      </div>
