...
function SupportFormModalComponent({
  ...
  const filteredUniversities = useMemo(() => {
    ...
    return universities.filter(u =>
      ...
      u.displayCode.toLowerCase().includes(query)
    ).slice(0, 10);
  }, [searchQuery]);
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div ref={dropdownRef} className="relative">
          ...
          {isDropdownOpen && filteredUniversities.length > 0 && (
            <div className="absolute z-50 w-full mt-1 bg-bg-card border border-border rounded-xl shadow-lg max-h-60 overflow-y-auto">
                ...
                >
                  <div>
                    <p className="font-medium text-text-primary">
                      <span className="text-xs text-primary mr-1.5">{university.displayCode}</span>
                      {university.name}
                    </p>
          ...
          {isDropdownOpen && searchQuery && filteredUniversities.length === 0 && (
            <div className="absolute z-50 w-full mt-1 bg-bg-card border border-border rounded-xl shadow-lg p-4 text-center text-text-muted">
              找不到符合的學校
            </div>
          )}
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            rows={3}
            className="w-full px-4 py-3 bg-bg-primary border border-border rounded-xl focus:outline-none focus:border-primary transition-all duration-200 resize-none text-text-primary placeholder:text-text-muted"
          />
        </div>
